...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.weight_input = WeightInput(
            self, self.weight_float, self.metric_bool)
        self.height_input = HeightInput(
            self, self.height_int, self.metric_bool)
...
class ResultText(ctk.CTkLabel):
    def __init__(self, parent, bmi_string):
        ...
        super().__init__(master=parent, text='22.5', font=font, textvariable=bmi_string)
...
class WeightInput(ctk.CTkFrame):
    def __init__(self, parent, weight_float, metric_bool):
        ...
        self.output_weight = ctk.StringVar(
            value=f'{str(weight_float.get())}Kg')
        label = ctk.CTkLabel(self, textvariable=self.output_weight,
                             text='70kg', text_color=BLACK, font=font)
        label.grid(row=0, column=2)

        # buttons
        minus_button = ctk.CTkButton(
            self, text='-', font=font, command=lambda: self.update_weight(('minus', 'large')), text_color=BLACK, fg_color=LIGHT_GRAY, hover_color=GRAY, corner_radius=6)
        minus_button.grid(row=0, column=0, sticky='ns', padx=8, pady=8)

        small_minus_button = ctk.CTkButton(
            self, text='-', font=font, command=lambda: self.update_weight(('minus', 'small')), text_color=BLACK, fg_color=LIGHT_GRAY, hover_color=GRAY, corner_radius=6)
        small_minus_button.grid(row=0, column=1, padx=4, pady=4)

        plus_button = ctk.CTkButton(
            self, text='+', font=font, command=lambda: self.update_weight(('plus', 'large')), text_color=BLACK, fg_color=LIGHT_GRAY, hover_color=GRAY, corner_radius=6)
        plus_button.grid(row=0, column=4, sticky='ns', padx=8, pady=8)

        small_plus_button = ctk.CTkButton(
            self, text='+', font=font, command=lambda: self.update_weight(('plus', 'small')), text_color=BLACK, fg_color=LIGHT_GRAY, hover_color=GRAY, corner_radius=6)
    ...
    def update_weight(self, info=None):
        ...
        if self.metric_bool.get():
            self.output_weight.set(
                f'{str(round(self.weight_float.get(), 2))}Kg')
        else:
...
class HeightInput(ctk.CTkFrame):
    def __init__(self, parent, height_int, metric_bool):
        ...
        slider = ctk.CTkSlider(master=self, command=self.update_text, button_color=GREEN,
                               button_hover_color=GRAY, progress_color=GREEN, fg_color=LIGHT_GRAY, variable=height_int, from_=100, to=250)
        slider.pack(side='left', fill='x', expand=True, pady=10, padx=10)

        self.output_string = ctk.StringVar()
        self.update_text(height_int.get())

        output_text = ctk.CTkLabel(self, textvariable=self.output_string, text='1.80m', text_color=BLACK, font=ctk.CTkFont(
            family=FONT, size=INPUT_FONT_SIZE))
...
class UnitSwitcher(ctk.CTkLabel):
    def __init__(self, parent, metric_bool):
        super().__init__(master=parent, text='metric', font=ctk.CTkFont(
            family=FONT, size=SWITCH_FONT_SIZE), text_color=DARK_GREEN)
